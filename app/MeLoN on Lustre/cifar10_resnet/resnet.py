...
def main(batch_size, depth, dataset):
    ...
    history = resnet.fit(data.train_images, data.train_labels, batch_size=batch_size, epochs=10, validation_split=0.2, verbose=0)
    score = resnet.evaluate(data.test_images, data.test_labels, verbose=0)

    print("=== batch_size is {0}, depth is {1}, dataset is {2}".format(batch_size, depth, dataset))
    print('Training Loss : ', history.history['loss'][-1])
    print('Training Acuuracy : ', history.history['acc'][-1])
    print('====================================================')
