...
tf.flags.DEFINE_string('data_dir', './input_data',
                       'Directory for storing input data')
#tf.flags.DEFINE_string('data_dir', 'hdfs://master.hadoop.com:8020/tmp/melon/input_data',
#                       'Directory for storing input data')
...
#tf.flags.DEFINE_string('working_dir', 'hdfs://master.hadoop.com:8020/user/hadoop/melon/working_dir',
tf.flags.DEFINE_string('working_dir', 'hdfs:///master.hadoop.com:8020/tmp/melon/'+os.environ["APP_ID"]+'/working_dir',
#tf.flags.DEFINE_string('working_dir', 'hdfs:///master.hadoop.com:8020/tmp',
                       'Directory under which events and output will be '
                       'stored (in separate subdirectories).')
#tf.flags.DEFINE_string('working_dir', os.getcwd()+'/working_dir',
#                       'Directory under which events and output will be '
#                       'stored (in separate subdirectories).')
#tf.flags.DEFINE_string('tb_dir', os.getcwd()+'/tb_dir', 'TensorBoard Directory')

# Training parameters
tf.flags.DEFINE_integer("steps", 200000,
...
def main(_):
    ...
    if fraction is not None:
        server_config = tf.ConfigProto(
            ...
            gpu_options=tf.GPUOptions(per_process_gpu_memory_fraction=float(fraction))
        )
    ...
    # worker면 task index마다 별개의 replica device에서 모델 생성
        ...
        if task_index is 0:  # chief worker
            ...
            os.system('hdfs dfs -mkdir /tmp/melon/'+os.environ["APP_ID"]+'/working_dir')
            os.system('hdfs dfs -chmod 777 /tmp/melon/'+os.environ["APP_ID"]+'/working_dir')

            logging.info('======getcwd()'+os.getcwd())
            #os.system('hdfs dfs -touchz /user/hadoop/melon/'+os.environ["APP_ID"]+'/test.txt')
        ...
        with tf.train.MonitoredTrainingSession(master=server.target,
            ...
            os.system('ls')
            os.system('ls ./input_data')
            os.system('mkdir ./input_data')
            os.system('hadoop fs -get /tmp/melon/mnist_distributed/input_data/* ./input_data/')
